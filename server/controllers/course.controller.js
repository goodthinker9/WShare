...
class CourseController {
  async getAll(req, res, next) {
    try {
      ...
      const [countResult] = await pool.query(
        `SELECT COUNT(*) as total FROM courses c WHERE c.is_active = 1`,
        []
      );
  ...
  async update(req, res, next) {
    try {
      const [courses] = await pool.query('SELECT id FROM courses WHERE id = ?', [req.params.id]);
      if (courses.length === 0) {
        throw new AppError('Course not found.', 404, 'COURSE_NOT_FOUND');
      }
      ...
      if (req.body.code !== undefined) { updates.push('code = ?'); params.push(req.body.code); }
      if (req.body.description !== undefined) { updates.push('description = ?'); params.push(req.body.description); }
      if (req.body.departmentId) { updates.push('department_id = ?'); params.push(req.body.departmentId); }
      if (req.body.academicLevelId) { updates.push('academic_level_id = ?'); params.push(req.body.academicLevelId); }
      if (req.body.semesterId) { updates.push('semester_id = ?'); params.push(req.body.semesterId); }
      if (req.body.creditHours !== undefined) { updates.push('credit_hours = ?'); params.push(req.body.creditHours); }
