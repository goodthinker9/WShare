...
class AdminService {
  ...
  async getDashboardStats() {
    ...
    const [verifiedStudents] = await pool.query(
      `SELECT COUNT(*) as count FROM users WHERE role = 'student' AND verification_status = 'approved' AND is_active = 1`
    );

    const [pendingVerifications] = await pool.query(
      `SELECT COUNT(*) as count FROM users WHERE role = 'student' AND verification_status = 'pending' AND is_active = 1`
    );

    const [pendingResources] = await pool.query(
      `SELECT COUNT(*) as count FROM resources WHERE status = 'pending' AND is_active = 1`
    );

    const [approvedResources] = await pool.query(
      `SELECT COUNT(*) as count FROM resources WHERE status = 'approved' AND is_active = 1`
    );
  ...
  async getStudentGrowth(startDate, endDate) {
    const [data] = await pool.query(
      `SELECT DATE(created_at) as date, COUNT(*) as count
       FROM users WHERE role = 'student' AND created_at BETWEEN ? AND ?
  ...
  async getDownloadTrends(startDate, endDate) {
    const [data] = await pool.query(
      `SELECT DATE(downloaded_at) as date, COUNT(*) as count
       FROM downloads WHERE downloaded_at BETWEEN ? AND ?
  ...
  async getUploadTrends(startDate, endDate) {
    const [data] = await pool.query(
      `SELECT DATE(created_at) as date, COUNT(*) as count
       FROM resources WHERE created_at BETWEEN ? AND ?
       GROUP BY DATE(created_at)
       ORDER BY date ASC`,
      [startDate, endDate]
    );
